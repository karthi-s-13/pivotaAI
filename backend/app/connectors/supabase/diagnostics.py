...
Runs structured, step-by-step diagnostic checks on Supabase database connectivity,
validating DNS, network pathways, TLS handshakes, Auth, and Metadata permissions.
"""
...
def run_diagnostics(config: SupabaseConnectionConfig) -> ConnectionTestResult:
    """Run 8 structured connection diagnostic steps for Supabase PostgreSQL databases."""
    step_config = ConnectionTestStep(name="configuration", status="pending")
    step_host = ConnectionTestStep(name="host_validation", status="pending")
    ...
    try:
        ...
        _skip_remaining(steps, ["host_validation", "dns", "network", "tls", "authentication", "database_access", "metadata_access"])
        return _make_failure_result(e, steps, start_total)

    # Step 2: Host & SSRF Validation check
    host_start = time.time()
    try:
        config.validate()  # Runs the SSRF host verification
        step_host.status = "success"
        step_host.latency_ms = round((time.time() - host_start) * 1000, 2)
    except Exception as e:
        step_host.status = "failed"
        err = InvalidConfigurationError(technical_details=f"SSRF validation blocked host: {str(e)}")
        step_host.message = err.safe_message
    ...
    # Establish full database connection to perform final checks
    ...
    try:
        ...
        # Classify psycopg2 error message into specific steps
